#!/usr/bin/env bash
# Remove packages that were installed from the historical Ubuntu 24.04 Ghostplant custom repo.
#
# This is intended to be run once inside an installed Ubuntu Tiny 24.04 system after
...
# Packages explicitly pulled from non-Ubuntu locations in older Ubuntu 24.04 build recipes.
...
Find and purge packages installed from the historical Ubuntu 24.04 Ghostplant custom APT repo,
...
if [[ ${#packages_to_purge[@]} -eq 0 ]]; then
  echo "No installed packages from the historical Ubuntu 24.04 Ghostplant custom repo were found."
else
